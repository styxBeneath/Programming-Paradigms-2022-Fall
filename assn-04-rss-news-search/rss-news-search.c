...
{            
  ...
  for (i = 0; i < strlen(ss); i++)  
    hashcode = hashcode * kHashMultiplier + tolower(ss[i]);  
  
  return hashcode % numBuckets;                                
}

static int caseCmpFn(const void * a, const void * b) {
  return strcasecmp(*(char **) a, *(char **) b);
}

void freeString(void* adr) {
  free(*(char **) adr);
}

void freeWord(void* adr) {
  word_data* data = (word_data*) adr;
  VectorDispose(&data->articles);
  free(data->word);
}

static void createStopWords(const char * filename) {
  FILE* file = fopen(filename, "r");
  streamtokenizer st;
  STNew(&st, file, kTextDelimiters, true);

  char buff[1024];
  while (STNextToken(&st, buff, sizeof(buff))) {
    char* dest = strdup(buff);
    HashSetEnter(&stop_words, &dest);
  }
...
int main(int argc, char **argv) {
  HashSetNew(&stop_words, sizeof(char*), 1009, StringHash, caseCmpFn, freeString);
  HashSetNew(&words, sizeof(word_data), 10007, StringHash, caseCmpFn, freeWord);
  createStopWords(stopWordsFile);
...
static int sameArticleCmpFn(const void* a, const void* b) {
  article* first = (article*) a;
  article* second = (article*) b;

  if (strcasecmp(first->article_name, second->article_name) > 0) return -1;
  if (strcasecmp(first->article_name, second->article_name) < 0) return 1;
  if (strcasecmp(first->url, second->url) > 0) return -1;
  if (strcasecmp(first->url, second->url) < 0) return 1;
  
  return 0;
}

void freeArticle(void* adr) {
  article* data = (article*) adr;
  free(data->url);
  free(data->article_name);
}
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
  ...
  if(HashSetLookup(&stop_words, &title) != NULL) {
    free(title);
    return;
  }
  HashSetEnter(&stop_words, &title);
  int numWords = 0;
  char word[1024];
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        ...
        if (HashSetLookup(&stop_words, &word_dup) != NULL) {
          numWords++; 
          free(word_dup); 
          continue; 
        }
        word_data* data = HashSetLookup(&words, &word_dup);
        free(word_dup);

        if (data) {
          article look_up;
          look_up.url = strdup(articleURL);
          look_up.article_name = strdup(articleTitle);
          int index = VectorSearch(&data->articles, &look_up, sameArticleCmpFn, 0, false);

          free(look_up.url);
          free(look_up.article_name);

          if (index >= 0) {
            article* search = VectorNth(&data->articles, index);
            int new_freq = search->word_freq + 1;
            search->word_freq = new_freq;
          } else {
            article new_article;
            new_article.url = strdup(articleURL);
            new_article.article_name = strdup(articleTitle);
            new_article.word_freq = 1;
            VectorAppend(&data->articles, &new_article);
          }
        } else {
          word_data new_word;
          new_word.word = strdup(word);
          VectorNew(&new_word.articles, sizeof(article), freeArticle, 0);
          ...
          VectorAppend(&new_word.articles, &new_article);
          HashSetEnter(&words, &new_word);
        }
        numWords++;
...
static int articleSortCmpFn(const void* a, const void* b) {
  article* first = (article*) a;
  article* second = (article*) b;

  if (first->word_freq > second->word_freq) return -1;
  if (first->word_freq < second->word_freq) return 1;
  if (strcasecmp(first->url, second->url) > 0) return -1;
  if (strcasecmp(first->url, second->url) < 0) return 1;
  if (strcasecmp(first->article_name, second->article_name) > 0) return -1;
...
void printFn(void* adr, void* auxdata) {
  int* n = (int*) auxdata;
  article* data = (article*) adr; 
  
  printf("%d.) \"%s\" [search term occurs %d ", *n, data->article_name, data->word_freq);
  data->word_freq > 1 ? printf("times]") : printf("time]");
...
static void ProcessResponse(const char *word) {
  if (WordIsWellFormed(word)) {
    ...
    int n = 1;
    VectorSort(&curr->articles, articleSortCmpFn);
    VectorMap(&curr->articles, printFn, &n);
  } else {
    printf(
