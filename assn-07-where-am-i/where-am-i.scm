...
(define (quicksort num-list foo)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) foo)))
	(append (quicksort (car split) foo) 
		(list (car num-list)) 
		(quicksort (cadr split) foo)))))
...
(define (intersection-points-fn seq fst)
  (if (null? seq) '()
    (append (intersect fst (car seq)) 
      (intersection-points-fn (cdr seq) fst))))

(define (intersection-points seq)
  (if (null? seq) '()
    (append (intersection-points-fn (cdr seq) (car seq))
      (intersection-points (cdr seq)))))


(define (distance-product point1 seq)
  (let ((sequenceCdr (remove point1 seq)))
    (apply * (map (lambda (point2) (dist point1 point2)) sequenceCdr))))


(define (rate-points seq)
  (if (null? seq) '()
    (map (lambda (elem)
      (list (distance-product elem seq) elem)) seq)))
...
(define (average-point-fn seq)
  (/ (apply + seq) (length seq)))

(define (average-point points)
  (let ((avg-pt (list (average-point-fn (map car points))
        (average-point-fn (map cadr points)))))
...
(define (where-am-i seq1 sec2)
  (if (or (null? seq1) (null? sec2)) '()
    (let ((guess (all-guesses seq1 sec2)))
      (sort-points (map best-estimate guess)))))
