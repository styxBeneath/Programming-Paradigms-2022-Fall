...
{
  sem_wait(&(bank->lock_bank));
  bank->active_workers--;
  if (bank->active_workers != 0) {
    sem_post(&(bank->lock_bank));
    sem_wait(&(bank->lock_workers[workerNum]));
    return 0;
  }
