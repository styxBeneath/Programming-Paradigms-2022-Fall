            ...
            AccountAmount initialAmount)
{
  ...
  for (int i = 0; i < numBranches; i++) {
    ...
    sem_init(&(branch->lock_balance), 0, 1);
    sem_init(&(branch->lock_branch), 0, 1);
