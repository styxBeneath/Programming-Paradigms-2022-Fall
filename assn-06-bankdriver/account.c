             ...
             AccountAmount initialAmount)
{
  ...
  sem_init(&(account->lock_balance), 0, 1);
  sem_init(&(account->lock_account), 0, 1);
  sem_init(&(account->lock_adjust), 0, 1);
...
{
  sem_wait(&(account->lock_balance));
  AccountAmount balance = account->balance; Y;
  sem_post(&(account->lock_balance));
  return balance;
}
