          ...
          int numWorkers)
{
  ...
  bank->lock_workers = malloc(numWorkers * sizeof(sem_t));
  for (int i = 0; i < numWorkers; i++) {
    sem_init(&(bank->lock_workers[i]), 0, 0);
  }

  sem_init(&(bank->lock_bank), 0, 1);
  sem_init(&(bank->lock_transfer), 0, 1);

  bank->active_workers = numWorkers;
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  unsigned int i;
  for (i = 0; i < bank->numberBranches; i++) {
    sem_wait(&(bank->branches[i].lock_branch));
  }
  ...
  *balance = bankTotal;
  ...
  for (i = 0; i < bank->numberBranches; i++) {
    sem_post(&(bank->branches[i].lock_branch));
  }
